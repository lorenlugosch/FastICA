	...
	while(fabs(fabs(dot_product)-2097152) > EPSILON) {
		...
		for (n = 0; n < N; n++) {
			...
			for (t = 0; t < T; t += 8) {
				...
				product_2[n] += multiply_Q6_10_by_Q6_10(whitened_signals[n][t+1], linear_tanh(product_1[t+1]));
				product_2[n] += multiply_Q6_10_by_Q6_10(whitened_signals[n][t+2], linear_tanh(product_1[t+2]));
				product_2[n] += multiply_Q6_10_by_Q6_10(whitened_signals[n][t+3], linear_tanh(product_1[t+3]));
				product_2[n] += multiply_Q6_10_by_Q6_10(whitened_signals[n][t+4], linear_tanh(product_1[t+4]));
				product_2[n] += multiply_Q6_10_by_Q6_10(whitened_signals[n][t+5], linear_tanh(product_1[t+5]));
				product_2[n] += multiply_Q6_10_by_Q6_10(whitened_signals[n][t+6], linear_tanh(product_1[t+6]));
				product_2[n] += multiply_Q6_10_by_Q6_10(whitened_signals[n][t+7], linear_tanh(product_1[t+7]));
			}	
		} 

		for (t = 0; t < T; t += 8) {
			sp[t] = linear_sech2(product_1[t]);
			sp[t+1] = linear_sech2(product_1[t+1]);
			sp[t+2] = linear_sech2(product_1[t+2]);
			sp[t+3] = linear_sech2(product_1[t+3]);
			sp[t+4] = linear_sech2(product_1[t+4]);
			sp[t+5] = linear_sech2(product_1[t+5]);
			sp[t+6] = linear_sech2(product_1[t+6]);
			sp[t+7] = linear_sech2(product_1[t+7]);
		}
