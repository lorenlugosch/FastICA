...
void fastica() {
	...
	while(fabs(fabs(dot_product)-2097152) > EPSILON) {
		...
		for (t = 0; t < T; t++) {
			product_1[t] = 0;
			product_1[t] += multiply_Q6_10_by_Q6_10(w_Q6_10[0],whitened_signals[0][t]);
			product_1[t] += multiply_Q6_10_by_Q6_10(w_Q6_10[1],whitened_signals[1][t]);
		}

		product_2[0] = 0; 
		for (t = 0; t < T; t++) {
			product_2[0] += multiply_Q6_10_by_Q6_10(whitened_signals[0][t], linear_tanh(product_1[t]));
		}	
		product_2[1] = 0; 
		for (t = 0; t < T; t++) {
			product_2[1] += multiply_Q6_10_by_Q6_10(whitened_signals[1][t], linear_tanh(product_1[t]));
		}	
