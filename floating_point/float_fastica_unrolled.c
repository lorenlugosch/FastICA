...
void rotate() {
	...
	product_2[0] = 0; 
	for (t = 0; t < T; t++) {
		product_2[0] += whitened_signals[0][t] * linear_tanh(product_1[t]);
	}
	product_2[1] = 0; 
	for (t = 0; t < T; t++) {
		product_2[1] += whitened_signals[1][t] * linear_tanh(product_1[t]);
	}
	...
	product_3[0] = w[0] * sum_1;
	product_3[1] = w[1] * sum_1;

	w_next[0] = (product_2[0] - product_3[0]) / N;
	w_next[1] = (product_2[1] - product_3[1]) / N;
}
